...
class Dialog {
    ...
    show(theme = "warning", backdrop = true) {
        ...
        if (this.buttons === "default") {
            $('#' + this.cloneId + ' .modal-footer').html(`<button type="button" class="btn btn-${theme}" ${this.framework.dismissattr}="modal">fix</button>`);
        } else {
...
$('.btn-create-group').on('click', function() {
    var dialog = new Dialog(
        ...
        <div class="form-floating mb-3">
            <input type="text" class="form-control custom-input" id="groupName" placeholder="Group Name">
    ...
    dialog.setButtons({
        'create': {
            name: 'Create',
            class: 'my-btn-secondary btn-create-group',
            onClick: function(event) {
                var modal = $(event.data.modal);
                var groupName = modal.find('#groupName').val().trim();
                var groupDesc = modal.find('#groupDesc').val().trim();
                if (groupName.length < 3 || groupDesc.length < 3) {
                    animateCSS('.btn-create-group', 'shakeX');
                    return;
                }
...
$('.btn-add-device').on('click', function() {
    $.get('/api/v1/dialog/api_keys_t', function(data) {
        ...
        dialog.setButtons([
            {
                name: 'Create',
                class: 'my-btn-secondary btn-create-device',
                onClick: function(event) {
                    var modal = $(event.data.modal);
